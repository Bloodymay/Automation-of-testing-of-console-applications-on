...
class TestPositive:
    def test_step1(self, make_folders, clear_folders, make_files, print_time):
        # test1
        res1 = checkout("cd {}; 7z a {}/arx -t{}".format(data["folder_in"], data["folder_out"], data["arx_type"]), "Everything is Ok")
    ...
    def test_step2(self, clear_folders, make_files):
        ...
        res.append(checkout("cd {}; 7z e arx.{} -o{} -y".format(data["folder_out"], data["arx_type"], data["folder_ext"]), "Everything is Ok"))
    ...
    def test_step3(self):
        # test3
        assert checkout("cd {}; 7z t arx.{}".format(data["folder_out"], data["arx_type"]), "Everything is Ok"), "test3 FAIL"

    def test_step4(self):
        # test4
        assert checkout("cd {}; 7z u arx2.{}".format(data["folder_in"], data["arx_type"]), "Everything is Ok"), "test4 FAIL"
    ...
    def test_step5(self, clear_folders, make_files):
        ...
        res.append(checkout("cd {}; 7z a {}/arx".format(data["folder_in"], data["folder_out"]), "Everything is Ok"))
        for i in make_files:
            res.append(checkout("cd {}; 7z l arx.{}".format(data["folder_out"], data["arx_type"], data["folder_ext"]), i))
    ...
    def test_step6(self, clear_folders, make_files, make_subfolder):
        ...
        res = []
        res.append(checkout("cd {}; 7z a {}/arx".format(data["folder_in"], data["folder_out"]), "Everything is Ok"))
        res.append(checkout("cd {}; 7z x arx.{} -o{} -y".format(data["folder_out"], data["arx_type"], data["folder_ext2"]), "Everything is Ok"))
        for i in make_files:
    ...
    def test_step7(self):
        # test7
        assert checkout("cd {}; 7z d arx.{}".format(data["folder_out"], data["arx_type"]), "Everything is Ok"), "test7 FAIL"
